...
import { ShoppingBag, Heart, ChevronLeft, ChevronRight, Expand } from "lucide-react";
...
export function ProductDetail({ phone }: { phone: Phone }) {
  ...
  const nextImage = () => setCurrentImage((i) => Math.min(images.length - 1, i + 1));
      ...
      >
        <div className="md:grid md:grid-cols-2 md:gap-12">
          ...
          <div>
            {/* Main image */}
            <div
              className="relative aspect-square bg-[#F8F8F8] rounded-2xl overflow-hidden cursor-zoom-in group"
            ...
            >
              ...
              {images.length > 0 && (
                <div className="absolute top-3 right-3 w-8 h-8 rounded-full bg-white/80 backdrop-blur-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-sm">
                  <Expand className="w-4 h-4 text-[#666666]" />
                </div>
              )}

              {/* Prev / Next inside main image */}
              {images.length > 1 && (
                <>
                  <button
                    onClick={(e) => { e.stopPropagation(); prevImage(); }}
                    disabled={currentImage === 0}
                    className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/90 flex items-center justify-center shadow-sm disabled:opacity-30 transition-opacity"
                  ...
                  <button
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/90 flex items-center justify-center shadow-sm disabled:opacity-30 transition-opacity"
              ...
              <div className="flex gap-2 mt-3 overflow-x-auto pb-1">
                {images.map((img, i) => (
                  <button
                    ...
                    className={`w-16 h-16 rounded-lg overflow-hidden flex-shrink-0 border-2 transition-colors ${i === currentImage ? "border-[#C9A84C]" : "border-transparent opacity-60 hover:opacity-100"}`}
                  >
          ...
          <div className="mt-6 md:mt-0">
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              {([
                ...
                phone.storage ? { label: "Almacenamiento", value: phone.storage } : null,
                phone.color ? { label: "Color", value: phone.color } : null,
            ...
            <div className="mt-8 flex gap-3">
              <button
                ...
                className={`flex-1 py-3.5 rounded-xl font-semibold text-sm transition-colors flex items-center justify-center gap-2 ${
                  inCart ? "bg-[#111111] text-white hover:bg-[#333]" : "bg-[#C9A84C] text-white hover:bg-[#A6862A]"
                }`}
              >
