...
export function AdminPhoneList({ phones: initial }: { phones: Phone[] }) {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (res.ok) setPhones((prev) => prev.filter((p) => p.id !== id));
  ...
  const handleToggle = async (id: string, field: "is_featured" | "is_available", value: boolean) => {
    ...
    if (res.ok) setPhones((prev) => prev.map((p) => (p.id === id ? { ...p, [field]: value } : p)));
    else alert("Error al actualizar. Intenta de nuevo.");
  };

  if (phones.length === 0) {
    return (
      <div className="bg-white rounded-2xl border border-[#E5E5E5] text-center py-12 text-[#666666]">
      ...
      <div className="md:hidden space-y-3">
        {phones.map((phone) => {
          ...
          return (
            <div
              key={phone.id}
              className={`bg-white rounded-2xl border border-[#E5E5E5] p-4 transition-opacity ${unavailable ? "opacity-60" : ""}`}
            >
              {/* Fila superior: imagen + info + precio */}
              <div className="flex gap-3">
                <div className="relative w-16 h-16 rounded-xl bg-[#F8F8F8] overflow-hidden flex-shrink-0">
                  {phone.images?.[0] && (
                    <Image src={phone.images[0]} alt={phone.title} fill className="object-cover" />
                  )}
                  {unavailable && (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <p className="font-semibold text-[#111111] text-sm leading-tight">{phone.title}</p>
                    {unavailable && (
                  ...
                  <p className="text-xs text-[#666666] mt-0.5">{phone.brand} · {phone.condition}</p>
                  {phone.storage && <p className="text-xs text-[#666666]">{phone.storage}</p>}
                  <p className={`text-sm font-bold mt-1 ${unavailable ? "text-gray-400 line-through" : "text-[#C9A84C]"}`}>
              ...
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-[#E5E5E5]">
                ...
                >
                  {phone.is_available
                    ? <><Eye className="w-4 h-4 text-emerald-500" /><span className="text-emerald-600">Disponible</span></>
                    : <><EyeOff className="w-4 h-4 text-gray-400" /><span className="text-gray-400">Oculto</span></>
                  }
                </button>
                ...
                >
                  <Star className={`w-4 h-4 ${phone.is_featured ? "fill-[#C9A84C] text-[#C9A84C]" : "text-[#E5E5E5]"}`} />
                  <span className={phone.is_featured ? "text-[#C9A84C]" : "text-[#666666]"}>
                    {phone.is_featured ? "Destacado" : "Destacar"}
                  </span>
                </button>
                ...
                <div className="flex gap-1">
                  <Link
                    ...
                    className="p-2 rounded-lg bg-[#F8F8F8] text-[#666666]"
                    aria-label="Editar"
                  >
                  ...
                  <button
                    ...
                    className="p-2 rounded-lg bg-[#F8F8F8] text-[#666666] hover:bg-[#FEE2E2] hover:text-[#D0021B] transition-colors"
                    aria-label="Eliminar"
                  >
